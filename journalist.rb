...
puts start_uppercase(my_array)

def uppercase(my_array)
  i = 0
  my_array.each do |item|
    if item =~ /[A-Z]/
  i += 1
...
puts uppercase(my_array)

def underscore(my_array)
  i = 0
  my_array.each do |item|
    if item =~ /(_)/
      n = item.count("_")
...
puts underscore(my_array)

def alphabetical(my_array)
  sort_my_array = my_array.map(&:downcase).sort { |a,b|
    a <=> b }
  return sort_my_array
end


puts alphabetical(my_array)
